...
namespace AuthApi.Controllers
{
            ...
            var participants = new List<ChatParticipant>
    {
        ...
        public async Task<IActionResult> DeleteUsersFromChat([FromBody] DeleteFromChatRequest request)
        {
            var usersToDelete = await _context.ChatParticipants
                .Where(u => request.UserID.Contains(u.userid) && u.chatid == request.ChatId)
                .ToListAsync();

            if (usersToDelete == null || !usersToDelete.Any())
            {
                return NotFound("Участники или чат не найдены");
            }

            _context.ChatParticipants.RemoveRange(usersToDelete);
            await _context.SaveChangesAsync();

            return Ok($"Успешно удалено {usersToDelete.Count} пользователей из чата");
        }
    }
}
